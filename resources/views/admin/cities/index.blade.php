@extends('admin.page', ['title' => "Администрирование - Города - Sellmecar"])

@section('content')
    <h1 class="text-center">Города</h1>
    <p><a href="{{ route('admin.cities.create') }}" class="btn btn-success"><i class="fa fa-plus"></i> Добавить</a></p>

    <div class="table-responsive">
        <table id="table_items" data-sortlist="[[1,0]]">
            <thead>
                <tr>
                    <th>Id</th>
                    <th>Город</th>
            ...
            <tbody>
                @foreach($items as $item)
                    <tr>
                        <td>{{ $item->id }}</td>
                        <td>{{ $item->name }}</td>
                        <td><a href="{{ route('admin.cities.edit', $item->id) }}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i></a></td>
                        <td>
                            {!! Form::open(['url' => route('admin.cities.destroy', $item->id), 'method' => 'DELETE']) !!}
