...
@section('content')
    ...
    <p><a href="{{ route('admin.faq.create') }}" class="btn btn-success"><i class="fa fa-plus"></i> Добавить</a></p>
            ...
            <tbody>
                @foreach($items as $item)
                    <tr>
                        <td>{{ $item->id }}</td>
                        <td>{{ $item->question }}</td>
                        <td>{{ str_limit(strip_tags($item->answer), 300) }}</td>
                        <td><a href="{{ route('admin.faq.edit', $item->id) }}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i></a></td>
                        <td>
                            {!! Form::open(['url' => route('admin.faq.destroy', $item->id), 'method' => 'DELETE']) !!}
